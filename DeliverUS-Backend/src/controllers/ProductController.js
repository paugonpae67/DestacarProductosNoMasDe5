...
const changeDestacado = async function (req, res) {
  try {
    const producto = await Product.findByPk(req.params.productId)

    if (producto.destacado) {
      producto.destacado = false
      producto.timeDestacado = null
    } else {
      const products = await Product.findAll({
        where: {
          restaurantId: req.params.restaurantId,
      ...
      if (products.length() > 5) {
        const p = await Product.findOne({
          ...
          order: [['timeDestacado', 'DESC']]
        })
        p.destacado = false
        p.timeDestacado = null
        await p.save()
      }
      producto.destacado = true
      producto.timeDestacado = new Date()
    }
    await producto.save()
    res.json(producto)
  } catch (err) {
