...
import { remove, changePromote } from '../../api/ProductEndpoints'
...
export default function RestaurantDetailScreen ({ navigation, route }) {
  ...
  const [productToBePromote, setProductToBePromote] = useState(null)
  ...
  const renderProduct = ({ item }) => {
    return (
      ...
      >
        ...
        <Pressable
            onPress={() => { setProductToBePromote(item) }}
            style={({ pressed }) => [
              {
                backgroundColor: pressed
                  ? GlobalStyles.brandGreenTap
          ...
          <View style={[{ flex: 1, flexDirection: 'row', justifyContent: 'center' }]}>
            <MaterialCommunityIcons name='delete' color={'white'} size={20}/>
            <TextRegular textStyle={styles.text}>
              {item.destacado ? 'Promote' : 'No Promote'}
            </TextRegular>
          </View>
        </Pressable>
  ...
  const patchProduct = async (product) => {
    try {
      await changePromote(product.restaurantId, product.id)
      await fetchRestaurantDetail()
      setProductToBePromote(null)
      showMessage({
        message: `Product ${product.name} succesfully promote`,
        type: 'success',
        style: GlobalStyles.flashStyle,
        titleStyle: GlobalStyles.flashTextStyle
      })
  ...
  return (
    <View style={styles.container}>
      ...
      <ConfirmationModal
        isVisible={productToBePromote !== null}
        onCancel={() => setProductToBePromote(null)}
        onConfirm={() => patchProduct(productToBePromote)}>
          <TextRegular>Promote.</TextRegular>
      </ConfirmationModal>
    </View>
  )
}
